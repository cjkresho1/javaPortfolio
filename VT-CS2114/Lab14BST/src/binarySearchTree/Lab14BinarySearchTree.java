    ...
    implements BinarySearchTree<T>
{
    ...
    {
        BinaryNode<T> node = find(x, root);
        if (node == null)
        {
            throw new ItemNotFoundException(x.toString());
        }
        remove(node.getElement(), root);
    }
    ...
    private BinaryNode<T> remove(T x, BinaryNode<T> node)
    {
        ...
        //Addresses the case when the root is the value to be removed
        if (root.getElement().compareTo(x) == 0)
        {
            //when the root is the only element in the tree
            if (root.getLeft() == null && root.getRight() == null)
            {
                this.makeEmpty();
                return null;
            }
            //when the root has a left leaf
            else if (root.getLeft() != null)
            {
                root.getLeft().setRight(root.getRight());
                root = root.getLeft();
            }
            //when the root has no left leaf
            else
            {
                root = root.getRight();
            }
        ...
        else
        {
            ...
            if (node.getLeft() != null && node.getRight() != null)
            {
                ...
                result.setLeft(remove(result.getElement(), node.getLeft()));
